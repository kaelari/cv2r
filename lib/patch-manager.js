...
class PatchGroup extends Array {
	constructor(name) {
		super();
		this.name = name;
		this.string = "";        // legacy string of 1-letter flags.
		this.flaglabels = null;  // new list of 3-letter flags.
		this.patches = [];
	}
...
class PatchManager {
    ...
    .map(normalizePatch);
			...
			const allPatchNames = (await this.getPatches()).map(p => p.id);
            const rawPatches = opts.patch.split(',').map(p => p.trim());

            const parsedPatches = rawPatches.map(entry => {
                ...
                rest.forEach(pair => {
                    ...
                    if (!k) return;
                    config[k.trim()] = parseValue(v);
                });

                if (!allPatchNames.includes(name) && !this.isPatchFile(name)) {
                    throw new Error(`invalid patch "${name}"`);
                }

                return { id: name, config };
            });

            this.optional.patches = parsedPatches
    .map(normalizePatch)
    .filter(Boolean);
	...
	getPatches(type = 'patch') {
		...
		for (let patch of fs.readdirSync(patchDir)) {
			type = type === 'patch' ? 'optional' : type;
			patches.push(this.getPatchData(type, patch.replace(/\.(?:js|json)$/, '')));
		}
	...
	getPatchData(ord, name) {
		// direct patch file
		if (this.isPatchFile(name)) {
			return require(name);
		}

		// check other patch locations by patch name, in order of precedence
		const patchPath = path.join(__dirname, '..', 'patch', ord);
		const files = [
			path.join(patchPath, `${name}.json`),
			path.join(patchPath, `${name}.js`),
			path.join(patchPath, name, 'index.json'),
			path.join(patchPath, name, 'index.js')
		];
		for (let i = 0; i < files.length; i++) {
			const file = files[i];
			try { fs.accessSync(file); } catch (err) { continue; }
			return require(file);
		}

		// if we got here, this isn't a patch file
		return null;
	}
