		...
		if (ord === 'optional') {
            if (isPre){
                for (patchName in patches) {
                    ...
                    var pat = allpatches.find(a => a.id == patches[patchName]);
                    if (pat != null) {
                        if (pat.flaglabel !== undefined){
                            if (pat.flaglabel.indexOf(',') == -1){
                                flaglabels.push(pat.flaglabel.toUpperCase());
                            }
                        }
                        else if (pat.character !== undefined ){
                            if (pat.character.indexOf(',') == -1){
                                flaglabels.push(pat.character.toUpperCase());
                            }
                        }
                        // TODO legacy 1-character patch labels should be removed eventually,
                        // but currently title "Tournament" etc labels still use this.
                        if (pat.character !== undefined ){
